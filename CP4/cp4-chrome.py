...
for i in range(0, len(contenido), 2):
    ...
    try:
        ...
        usr = d.find_element(By.CSS_SELECTOR, ".alert > .text-center").text
        if usr == "username is required":
            accepted+=1
        else:
            rejected+=1
...
with open("reportecp4-chrome.txt", "w") as file:
    file.write("")

archivo2 = open("reportecp4-chrome.txt", "r+")
